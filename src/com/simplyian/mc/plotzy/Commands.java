...
public class Commands {
    ...
    public boolean pz(CommandSender sender, String alias, String args[]) {
        if (args.length > 0) {
            ...
            } else if (function.equalsIgnoreCase("about")) {
            ...
            } else if (function.equalsIgnoreCase("forsale")) {
                pzForSale(sender, newArgs);
            } else if (function.equalsIgnoreCase("buy")) {
                pzBuy(sender, newArgs);
            } else if (function.equalsIgnoreCase("test")) {
    ...
    public void pzCreate(CommandSender sender, String[] args) {
        if (sender instanceof Player) {
            Player player = (Player) sender;
            if (player.hasPermission("plotzy.plot.create")) {
                if (args.length > 0) {
                    String plotName = argString(args);
                    double cost = PlotFunctions.getDefaultCreateCost();
                    if (MoneyHandler.get(player.getName()) >= cost) {
    ...
    public void pzDelete(CommandSender sender, String[] args) {
        if (sender instanceof Player) {
            Player player = (Player) sender;
            if (player.hasPermission("plotzy.plot.delete")) {
    ...
    public void pzExpand(CommandSender sender, String[] args) {
        if (sender instanceof Player) {
            Player player = (Player) sender;
            if (player.hasPermission("plotzy.plot.expand")) {
                Plot plot = PlotFunctions.inWhichPlot(player.getLocation());
                if (plot != null) {
                    if (plot.canExpand(player) == true) {
                        ...
                        if (plotSize + byHowMuch <= PlotFunctions.getMaxSizeAllowed()) {
                            ...
                            String playerName = player.getName();
                            if (MoneyHandler.get(playerName) >= expandCost) {
                                MoneyHandler.subtract(playerName, expandCost);
    ...
    public void pzShrink(CommandSender sender, String[] args) {
        if (sender instanceof Player) {
            Player player = (Player) sender;
            if (player.hasPermission("plotzy.plot.shrink")) {
                Plot plot = PlotFunctions.inWhichPlot(player.getLocation());
                if (plot != null) {
        ...
        if (sender instanceof Player) {
            ...
            if (Plotzy.hasPermission(player, "plotzy.info")) {
                ...
                if (plot != null) {
                    ...
                    sender.sendMessage(ChatColor.GOLD + "--" + plot.getName() + "--");
                    sender.sendMessage(ChatColor.YELLOW + "Size: " + plot.getSize());
                } else {
    ...
    /**
     * Puts the plot stood in up for sale.
     * 
     * @param sender
     * @param args 
     * 
     * @since 0.4
     */
    public void pzForSale(CommandSender sender, String[] args) {
        if (sender instanceof Player) {
            Player player = (Player) sender;
            if (args.length > 0) {
                int salePrice;
                try {
                    salePrice = Integer.parseInt(args[0]);
                } catch (NumberFormatException ex) {
                    sender.sendMessage(args[0] + " is not a valid price to sell a plot at.");
                    return;
                }
                if (Plotzy.hasPermission(player, "plotzy.forsale")) {
                    Plot plot = PlotFunctions.inWhichPlot(player.getLocation());
                    if (plot != null) {
                        plot.setFlag("saleprice", salePrice);
                    } else {
                        sender.sendMessage(ChatColor.RED + "You are not in a plot!");
                    }
                } else {
    ...
    public void pzBuy(CommandSender sender, String[] args) {
        if (sender instanceof Player) {
            Player player = (Player) sender;
            if (Plotzy.hasPermission(player, "plotzy.buy")) {
                Plot plot = PlotFunctions.inWhichPlot(player.getLocation());
                if (plot != null) {
                    if (plot.flagExists("saleprice")) {
                        
                    } else {
                        sender.sendMessage(ChatColor.RED + "This plot is not currently for sale.");
                    }
                } else {
                    sender.sendMessage(ChatColor.RED + "You are not in a plot!");
                }
