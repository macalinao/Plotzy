...
public class PlotFunctions {
    ...
    public static Plot inWhichPlot(Location loc) {
        for (Plot plot : pl.getPlotList().values()) {
    ...
    public static Plot createPlot(String plotName, Location center, int size) {
        if (pl.isMysql() == true) Database.execute("INSERT INTO " + Database.prefix + "plotzy_plots VALUES (0, '" + plotName + "', '" + size + "', '" + center.getWorld().getName() + "', '" + center.getBlockX() + "', '" + center.getBlockY() + "', '" + center.getBlockZ() + "')");
        pl.getPlotList().put(plotName, new Plot(plotName, center, size));
        return pl.getPlotList().get(plotName);
    }
    ...
    public static boolean plotExists(String plotName) {
        return pl.getPlotList().containsKey(plotName) ? true : false;
    }
    ...
    public static void deletePlot(Plot plot) {
        plot.delete();
        pl.getPlotList().remove(plot.getName());
    }
}